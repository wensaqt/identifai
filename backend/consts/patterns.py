...
EXTRACT_SIREN = r"(?:SIREN|RCS\s+\S+)\s*[:\-]?\s*(\d{9})\b"
EXTRACT_COMPANY_NAME = (
    r"\b([A-ZÀ-Ÿ][A-Za-zÀ-ÿ0-9&.,'’\-\s]{2,100}?(?:\s+(?:SARL|SAS|SA|LLC|Ltd|Inc))?)\b"
)
EXTRACT_TVA = r"\b(FR[A-Z0-9]{2}\d{9})\b"
EXTRACT_IBAN = r"\b(FR\d{2}[\s\d]{20,30})\b"
EXTRACT_BIC = r"\bBIC\s*[:\-]?\s*([A-Z]{6}[A-Z0-9]{2}(?:[A-Z0-9]{3})?)\b"
EXTRACT_MONTANT_HT = (
    r"(?:total\s+HT|montant\s+HT|HT)\s*[:\-]?\s*([\d\s]+[.,]\d{2})\s*[€E]?"
)
EXTRACT_MONTANT_TTC = (
    r"(?:total\s+TTC|montant\s+TTC|TTC)\s*[:\-]?\s*([\d\s]+[.,]\d{2})\s*[€E]?"
)
EXTRACT_MONTANT_TVA = r"TVA\s+\d+%?\s*[:\-]?\s*([\d\s]+[.,]\d{2})\s*[€E]?"
EXTRACT_TVA_RATE = r"TVA\s+(\d+)\s*%"
...
EXTRACT_METHODE = (
    r"[Mm][ée]thode\s*[:\-]?\s*(virement|pr[ée]l[èe]vement|ch[èe]que|carte\s+bancaire)"
)
EXTRACT_PERIODE = r"\b(\d{4}-T[1-4])\b"
EXTRACT_CA_DECLARE = (
    r"[Cc]hiffre\s+d.affaires\s+d[ée]clar[ée]\s*[:\-]?\s*([\d\s,.]+)\s*[€E]?"
)
EXTRACT_DATE_DECLARATION = r"[Dd]ate\s+de\s+d[ée]claration\s*[:\-]?\s*({date})".format(
    date=EXTRACT_DATE
)
...
CLASSIFY_URSSAF_CERTIFICATE = [r"urssaf", r"attestation"]
CLASSIFY_SIRET_CERTIFICATE = [r"sirene|siret", r"avis\s+de\s+situation|r[ée]pertoire"]
CLASSIFY_COMPANY_REGISTRATION = [
    r"extrait\s*k\s*bis|k\s*bis|greffe|tribunal\s+de\s+commerce"
]
CLASSIFY_BANK_ACCOUNT_DETAILS = [r"relev[ée]\s+d.identit[ée]\s+bancaire|rib", r"iban"]
CLASSIFY_PAYMENT = [r"confirmation\s+de\s+paiement"]
CLASSIFY_INVOICE = [r"facture"]
CLASSIFY_QUOTE = [r"devis"]
